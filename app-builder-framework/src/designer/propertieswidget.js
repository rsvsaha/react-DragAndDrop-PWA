...
export const PropertiesWidget = (props) => {
    ...
    return(
        <div style={{float:"right",width:"200px",height:"576px",
        border:"2px solid black"}}>
              <div>Selected Component Id:{selectedId}</div>
                {
                ...
                {
                    (selectedId !== null ) ? <div><button onClick={()=>{console.log("Saved",stateObject);props.saveState({id:selectedId,stateObject:stateObject}); dispatch(disselectionAction());}}>SAVE</button>
                    <button onClick={()=>{console.log("Deleted",stateObject);props.deleteState(selectedId); dispatch(disselectionAction());}}>DELETE</button></div> : null
                }
